...
- Foreign outbound soft-disable (CN): no Google Fonts / GA / Meta Pixel / Turnstile api.js in HTML (Turnstile injects only when `VITE_TURNSTILE_SITEKEY` is set); GitHub API and Solana RPC off unless `GITHUB_OUTBOUND_ENABLED=1` / `SOLANA_OUTBOUND_ENABLED=1`; Claudium economy, daily-rewards payout service, and perf-report ingestion off unless `CLAUDIUM_OUTBOUND_ENABLED=1` / `DAILY_REWARDS_OUTBOUND_ENABLED=1` / `PERF_REPORT_ENABLED=1`; wallet + GitHub + Claudium + Daily Rewards UI default off (`VITE_WALLET_DISABLED` / `VITE_GITHUB_DISABLED` / `VITE_CLAUDIUM_DISABLED` / `VITE_DAILY_REWARDS_DISABLED` / `VITE_PERF_REPORT_DISABLED`)
