...
describe('secondary affix roll', () => {
  ...
  it('survives clone with the live-player secondary JSON shape from origin/dev', () => {
    // Live exclusive-server inventories already persist instance.secondary as
    // { versatilityRating?, critRating?, hasteRating? }. Keep that wire/save
    // shape byte-compatible so existing online characters load unchanged.
    const liveShape = {
      secondary: { versatilityRating: 27, critRating: 18, hasteRating: 9 },
    };
    const cloned = cloneItemInstancePayload(liveShape);
    expect(cloned.secondary).toEqual(liveShape.secondary);
    expect(cloned.secondary).not.toBe(liveShape.secondary);
    expect(Object.keys(cloned.secondary ?? {}).sort()).toEqual([
      'critRating',
      'hasteRating',
      'versatilityRating',
    ]);
  ...
  it('survives corpse loot into inventory (grant preserves instance)', () => {
    ...
    if (!player) return;
    sim.players.get(pid)!.inventory.splice(0, sim.players.get(pid)!.inventory.length);
